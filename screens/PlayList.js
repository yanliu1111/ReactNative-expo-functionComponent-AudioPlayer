...
const PlayList = ({ navigation }) => {
  ...
  const renderPlayList = async () => {
    ...
    if (result === null) {
      ...
      context.setPlayList(newPlayList);
      return await AsyncStorage.setItem(
        "playlist",
        JSON.stringify(newPlayList)
      );
    }
    context.setPlayList(JSON.parse(result));
  };
  ...
  const handleBannerPress = async (playList) => {
    if (addToPlayList) {
      ...
      if (sameAudio) {
        ...
        sameAudio = false;
        return context.setAddToPlayList(null);
      }
      context.setAddToPlayList(null);
      context.setPlayList(updatedList);
